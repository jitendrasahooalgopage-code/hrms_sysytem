                                    ...
                                    <div class="d-flex justify-content-center align-items-center gap-2">
                                        ...
                                        @if (in_array($role, ['super-admin', 'administrator']))
                                            ...
                                            @if (($loggedInEmployeeId && $leave->current_approver_id == $loggedInEmployeeId) || !in_array($leave->status, [1, 4]))
                                                ...
                                                <button type="button" class="btn btn-outline-info btn-sm" data-bs-toggle="modal" data-bs-target="#historyModal{{ $leave->id }}">
                                                    <i class="fas fa-history"></i>
                                                </button>

                                                {{-- Workflow History Trail Modal --}}
                                                <div class="modal fade text-left" id="historyModal{{ $leave->id }}" tabindex="-1">
                                                    <div class="modal-dialog modal-dialog-centered">
                                                        <div class="modal-content">
                                                            <div class="modal-header">
                                                                <h5 class="modal-title">Leave History — {{ $leave->employee->firstname }}</h5>
                                                                    ...
                                                                    <div class="d-flex align-items-start mb-3">
                                                                        <div class="me-3 mt-1">
                                                                            ...
                                                                            @elseif($history->action === 'forwarded')
                                                                                <span class="badge bg-primary"><i class="fas fa-arrow-right"></i></span>
                                                                            @else
                                        ...
                                        @if (in_array($role, ['super-admin', 'administrator']))
                                            <form action="{{ match ($role) {
                                                ...
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-outline-danger btn-sm" onclick="return confirm('Delete this leave record?')">
                                        ...
                                        @if (!($loggedInEmployeeId && $leave->current_approver_id == $loggedInEmployeeId && !in_array($leave->status, [1, 4])) && !in_array($role, ['super-admin', 'hr', 'administrator']))
                                            <span class="text-muted">—</span>
                                        @endif
